...
def evaluate_metrics(loader, model, device):
    ...
    for i, (img, label) in enumerate(loader):
        ...
        for j in range(len(out)):
            ssim_sum += skimage.metrics.structural_similarity(
                img[j, 0], out[j, 0], data_range=1.0, multichannel=False
            )
            mse_sum += np.mean((out[j] - img[j]) ** 2)
            rmse_sum += np.sqrt(np.mean((out[j] - img[j]) ** 2))

        num_samples += len(out)

    avg_ssim = ssim_sum / num_samples
    avg_mse = mse_sum / num_samples
    avg_rmse = rmse_sum / num_samples

    return avg_ssim, avg_mse, avg_rmse
...
def train(epoch, loader, model, optimizer, scheduler, device):
    ...
    for i, (img, label) in enumerate(loader):
        ...
        if dist.is_primary():
            ...
            if i % 100 == 0:
                model.eval()
                nii_img = sitk.ReadImage('./test/BraTS2021_00621_t1.nii.gz')
                # 获取数据和元数据
                nii_data = sitk.GetArrayFromImage(nii_img)
                nrrd_img = sitk.GetImageFromArray(nii_data)
                nrrd_img = resize_image_itk(nrrd_img, (256, 256, 256),
                ...
                nrrd_file = '/root/autodl-tmp/data/sample_t1_829/BraTS2021_00621_t1.nii.gz'
                ...
                output_filename = f"/root/autodl-tmp/data/sample_t1_829/{str(epoch + 1).zfill(5)}_{str(i).zfill(5)}.nii.gz"
...
def main(args):
    ...
    loader = DataLoader(
        dataset, batch_size=1 // args.n_gpu, sampler=sampler, num_workers=12
    )
...
#             mse_n += part["mse_n"]
#
#         avg_ssim, avg_mse, avg_rmse = evaluate_metrics(loader, model, device)
#
#         if dist.is_primary():
#             lr = optimizer.param_groups[0]["lr"]
#
...
#                 (
#                     f"epoch: {epoch + 1}; mse: {recon_loss.item():.5f}; "
#                     f"latent: {latent_loss.item():.3f}; avg mse: {avg_mse:.5f}; "
#                     f"avg rmse: {avg_rmse:.5f}; avg ssim: {avg_ssim:.5f}; "
#                     f"lr: {lr:.5f}"
#                 )
#             )
#             if i % 100 == 0:
...
#                 #                 image_path_resize='/root/autodl-tmp/data/sample_test/BraTS2021_00621_t2.nii.gz'
#
...
#                 #                 dataset_sample=NiFTIDataset("/root/autodl-tmp/data/sample_test")
...
#                 #                         out_image = sitk.GetImageFromArray(out.cpu().numpy())  # Move the tensor to CPU before conversion
...
#                 #                 model.eval()
#                 #                 image_path = "./test/BraTS2021_00621_t2.nii.gz"
#                 #                 image_path_resize='./sample/BraTS2021_00621_t2.nii.gz'
#                 #                 sitk_image = sitk.ReadImage(image_path)
#                 #                 sitk_image = resize_image_itk(sitk_image, (256, 256, 256),resamplemethod=sitk.sitkLinear)
#                 #                 sitk.WriteImage(sitk_image, image_path_resize)
#
#                 #                 # Convert the SimpleITK image to a numpy array
#                 #                 numpy_image = sitk.GetArrayFromImage(sitk_image)
#                 #                 numpy_image = torch.tensor(numpy_image, dtype=torch.float32)
#                 #                 numpy_image = numpy_image.unsqueeze(0)
#                 #                 # Modify this part to use your fixed sample
#                 #                 sample = numpy_image.to(device)  # Load your fixed sample tensor here
#
#                 #                 with torch.no_grad():
#                 #                     out, _ = model(sample)
#
#                 #                 # Convert the PyTorch tensor to a SimpleITK image
#                 #                 out_image = sitk.GetImageFromArray(out.cpu().numpy())  # Move the tensor to CPU before conversion
#                 #                 output_filename = f"sample/{str(epoch + 1).zfill(5)}_{str(i).zfill(5)}.nii.gz"
#                 #                 sitk.WriteImage(out_image, output_filename)
#
#                 #                 model.train()
#
...
#                 nii_direction = nii_img.GetDirection()
#                 nii_origin = nii_img.GetOrigin()
#                 nrrd_file = '/root/autodl-tmp/data/sample_test/BraTS2021_00621_t2.nii.gz'
#                 sitk.WriteImage(nrrd_img, nrrd_file)
#                 # 扩展维度并复制通道
#                 sample = torch.tensor(sample, dtype=torch.float32)
#                 sample = sample.unsqueeze(0)  # 有这一行，我只加了一个维度，那conv3d怎么传进去4d的？
...
#                     out = out.squeeze(0)  # 下午那个效果还可以的没有这一行
#                     # print(out.shape)
#                 # out_file = f"/root/autodl-tmp/sample2/{str(epoch + 1).zfill(5)}_{str(i).zfill(5)}.nii.gz"
#
